...
class TestMpLogin:
    ...
    def test_mp_login_sucess(self, username, password, expect_role, exe_driver):
        # 调用登录业务方法
        exe_driver.page_mp_login(username, password)
        print(exe_driver.page_get_alert())
        try:
            assert expect_role == exe_driver.page_get_role()
    ...
    def test_mp_login_error_1(self, username, password, expect_alert, exe_driver):
        ...
        exe_driver.page_mp_login(username, password)
        try:
            assert expect_alert == exe_driver.page_get_alert()
        except Exception as e:
            # 输出错误信息
            log.error("断言出错，错误信息: {}".format(e))
            print("错误原因：", e)
    ...
    def test_mp_login_error_2(self, username, password, expect_alert, exe_driver):
        ...
        exe_driver.page_mp_login(username, password)
        try:
            assert expect_alert == exe_driver.page_get_form_item_error()
        except Exception as e:
            # 输出错误信息
            log.error("断言出错，错误信息: {}".format(e))
            print("错误原因：", e)
